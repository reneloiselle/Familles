-- Recherche produit sans création + option création explicite à l'ajout en liste
...
BEGIN
  ...
  v_product_id := find_product(p_family_id, p_name, p_upc);
...
BEGIN
  ...
  LOOP
    ...
    IF p_link_products THEN
      v_product_id := find_product(v_family_id, v_line, NULL);

      IF v_product_id IS NULL AND p_create_if_missing THEN
...
GRANT EXECUTE ON FUNCTION find_product TO authenticated;
